...
import { ModelId, TelegramUserId, UserFullData, ViewObject, ViewType, modelId } from 'shared/types'
...
export const App = () => {
	...
	const loadUser = useCallback(async (id: number) => {
		try {
			const user = await toast.promise(api.users.get(modelId.parse(id)), {
	...
	useEffect(() => {
		...
		api.auth.login(telegramUserId as unknown as TelegramUserId).then(() => {
