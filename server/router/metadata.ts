...
const validateParams =
	(schema: ZodSchema) => async (req: Request, res: Response, next: NextFunction) => {
		try {
			...
			res.status(400).json({
				error: req.t('server.invalid_params', { path: (error as ZodError).errors[0].path.join('.') }),
			})
...
const validateBody =
	(schema: ZodSchema) => async (req: Request, res: Response, next: NextFunction) => {
		try {
			...
			res.status(400).json({
				error: req.t('server.invalid_argument', { path: (error as ZodError).errors[0].path.join('.') }),
			})
